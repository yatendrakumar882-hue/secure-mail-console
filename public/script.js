document.addEventListener('DOMContentLoaded', () => {
  ...
  if (toggleGatePassword && gatePassword) {
    toggleGatePassword.addEventListener('click', () => {
      const isPass = gatePassword.getAttribute('type') === 'password';
      gatePassword.setAttribute('type', isPass ? 'text' : 'password');
      toggleGatePassword.innerHTML = isPass ? '<i class="fa-regular fa-eye-slash"></i>' : '<i class="fa-regular fa-eye"></i>';
    });
  }
  ...
  if (togglePasswordBtn && dashboardPassword) {
    togglePasswordBtn.addEventListener('click', () => {
      const isPass = dashboardPassword.getAttribute('type') === 'password';
      dashboardPassword.setAttribute('type', isPass ? 'text' : 'password');
  ...
  function startUI(total) {
    ...
    if (statusText) statusText.textContent = 'Sending cleanly to inbox...';
  ...
  function updateUI(sent, failed, total, label) {
    if (statSent) statSent.textContent = sent;
    if (statFailed) statFailed.textContent = failed;

    const processed = sent + failed;
    if (statRemaining) statRemaining.textContent = Math.max(0, total - processed);

    const pct = Math.min(100, Math.round((processed / total) * 100));
    if (progressBar) progressBar.style.width = `${pct}%`;

    if (label && statusText && isSending && !stopRequested) {
  ...
  if (sendBtn) {
    sendBtn.addEventListener('click', async () => {
      ...
      if (extractedEmails.length === 0) {
        ...
        return alert('Please enter at least one valid recipient email.');
      }
      ...
      try {
        ...
        const recipientsList = [...extractedEmails];
        startUI(recipientsList.length);
          ...
          for (const line of lines) {
            if (line.startsWith('data: ')) {
              ...
              try {
                ...
                if (item.success) {
                  sentCount++;
                  updateUI(sentCount, failedCount, recipientsList.length, `Sent: ${item.recipient}`);
                } else {
                  failedCount++;
                  updateUI(sentCount, failedCount, recipientsList.length, `Failed: ${item.recipient}`);
                }
              } catch {
        ...
        if (stopRequested) {
          ...
          if (statusText) statusText.textContent = 'Process halted by user.';
        } else {
          if (statusIcon) statusIcon.className = 'fa-solid fa-circle-check text-success';
          if (statusText) statusText.textContent = 'Finished successfully!';
          alert(`Completed!\nDelivered: ${sentCount}\nFailed: ${failedCount}`);
        }
      } catch (err) {
        console.error(err);
        alert('Network or server error during sending.');
      } finally {
        isSending = false;
        resetUI();
      }
  ...
  if (stopBtn) {
    stopBtn.addEventListener('click', async () => {
      ...
      if (statusText) statusText.textContent = 'Stopping remaining queue...';
